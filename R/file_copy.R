...
file_copy <- function(from, to, args = c("-p", "-r")) {
  ...
  if(length(from) == 1) {
    ...
    if(length(to) > 1) stop("More `from` elements than `to`")
    return(cp(from, to, args))

  } else if(length(from) > 1 && length(to) == 1) {

    # Copy multiple files to a single directory
    if(!dir.exists(to)) stop("Multiple `from` provided with only one `to`, but `to` is not an existing directory.")
    return(vapply(
      X = from,
      FUN = cp,
      FUN.VALUE = logical(1),
    ...
    if(length(from) != length(to)) stop("Multiple `from` and `to` provided, but they are not of equal length.")
    return(mapply(
      FUN = cp,
      from = from,
      to = to,
