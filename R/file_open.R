...
.file_open <- function(path, no_copies = FALSE) {
  ...
  if(dir.exists(path)) {
    message("File is a directory: ", path)
    return(FALSE)
  }
  ...
  if(no_copies && identical(open_function, file.show)) {
    message("Cannot open an editable version. Printing Windows path: ", path_to_windows(path))
    return(FALSE)
  }

  # Check if file is binary and extension is not expected
  if(is_binary(path) && identical(open_function, rstudioapi::navigateToFile)) {
    message("File is binary and cannot be opened: ", path)
    return(FALSE)
  }
